...
# A signal is an automation mechanism that allows the program to execute code in the
# form of a callback function when an event occurs. The signal is the event that
...
def create_root_project(sender, instance, created, **kwargs):
  ...
  if created:
    Project.objects.create(owner=instance, title="Root", parent=None)
...
def track_visibility_change(sender, instance, **kwargs):
  ...
  if instance.id is None:
    ...
    return
  original = Project.objects.get(id=instance.id)
...
def propagate_visibility(sender, instance, created, **kwargs):
  ...
  unchanged.
  """
  if created or not instance.visibility_changed:
    return
  descendants = instance.bfs(include_root=False)
  if descendants:
...
def override_child_permissions(sender, instance, **kwargs):
  ...
  """
  descendants = instance.project.bfs(include_root=False)
  if not descendants:
    return
  ProjectPermission.objects.filter(
    project_id__in=descendants, user=instance.user
  ).delete()
